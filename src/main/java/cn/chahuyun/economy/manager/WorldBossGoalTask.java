...
public class WorldBossGoalTask implements WorldBossTask {
    @Override
    public void execute() {
        ...
        for (Map.Entry<Long, List<WorldBossUserLog>> m : userLogMap.entrySet()) {
            ...
            if (bb > 0) {
                sb.append("-------").append("\r\n");
                sb.append("获取wdit bb 奖金如下：").append("\r\n");
                userIdList.forEach(userId -> {
                    ...
                    if (!EconomyUtil.plusMoneyToUser(member, bb)) {
                        ...
                        sb.append(new At(userId).getDisplay(group)).append(" 金额：").append(bb).append("\r\n");
                    }
                });
                sb.append("-------").append("\r\n");
            }
            ...
            sb.append("-------").append("\r\n");
            WorldBossConfigManager.deleteAllWorldBossUserLog();
            messgae.plus(sb.toString());
    ...
    private void getWorldPropInfo(Group group, Set<Long> userIdList, StringBuilder sb, String propCode, int num) {
        ...
        propProbabilityUserId.forEach(userId -> {
            ...
            if (!userInfo.addPropToBackpack(userBackpack)) {
                ...
                sb.append(new At(userId).getDisplay(group)).append(" 获取道具：").append(propsInfo.getName()).append("\r\n");
            }
        });
    }
